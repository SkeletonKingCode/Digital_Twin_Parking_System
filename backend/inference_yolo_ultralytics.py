...
def extract_timestamp(filepath, mode='filename_ic'):
    """
    Extract timestamp from filename like '2015-11-12_0709.jpg'
    Returns a string like '2015-11-12_0709' or formatted datetime.
    """
    import re
    import os
    basename = os.path.basename(filepath)
    # Match pattern: YYYY-MM-DD_HHMM.jpg
    match = re.search(r'(\d{4}-\d{2}-\d{2}_\d{4})', basename)
    if match:
        timestamp_str = match.group(1)  # e.g., '2015-11-12_0709'
        if mode == 'filename_ic':
            ...
            for img_ in image_list:
                ...
                if 'results' not in img_:
                    ...
                    with Image.open(img_) as img_object:
                        ...
                        df.at[df.index[-1], 'processing_time'] = str(elapsed_time)
